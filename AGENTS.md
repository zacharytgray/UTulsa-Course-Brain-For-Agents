<!-- generated from CLAUDE.md by hermes-codex-context; edit CLAUDE.md and run `hermes-codex-context repos apply --yes` -->
<!-- regenerate with `~/GitHub/hermes-dispatch/bin/hermes-codex-context repos apply --yes` -->
...
These are not commands you can call. Each one is written out in its own `.claude/skills/<name>/SKILL.md`: read that file and follow the steps by hand.
