...
- `scripts/blackboard-mirror.py [class-code]` refreshes the mirror. The scheduled job runs it on every sync, so `harvey/` is normally already current.
...
- Completion is the one thing that flows Todoist → file: check a task off and the next scheduled sync flips that assignment to `status: submitted` and adds a `## Submitted` section noting the date.
